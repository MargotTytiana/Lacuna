...
## 第一可数空间
在点集拓扑理论中，第一可数空间（first countable space）是一类定义了某种可数公理的拓扑空间，常常称为${\displaystyle C_{1}}$空间。
如果拓扑空间${\displaystyle X}$中任意一点都有可数的邻域基，我们就称这个空间为**第一可数空间**，或称${\displaystyle C_{1}}$空间、${\displaystyle A_{1}}$空间。例如，度量空间是第一可数空间。

## 第二可数空间
如果一个拓扑空间${\displaystyle X}$有可数的拓扑基，我们就称它是第二可数空间，或称完全可分空间，也称${\displaystyle C_{2}}$空间或${\displaystyle A_{2}}$空间。   
我们感兴趣的大多数拓扑空间是第二可数的。  
例如， ${\mathbb{R}}^n$ 是第二可数的，因为它是由可数的邻域基组成的——**开矩形的边长是有理数**，并且所有的点的坐标都是有理数。显然，每个第二可数空间也是第一可数的，但反之不然。  
